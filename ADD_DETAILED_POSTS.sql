-- Thêm các bài viết chi tiết về công nghệ
INSERT INTO posts (
  id, author_id, title, slug, excerpt, content, image_url, status, published_at, created_at, updated_at
) VALUES
...
(
  ...
  'Supabase là gì? Hướng dẫn BaaS mã nguồn mở',
  'supabase-la-gi-huong-dan-baas-ma-nguon-mo',
  'Supabase là một Backend-as-a-Service mã nguồn mở, được xây dựng trên PostgreSQL. Hãy khám phá các tính năng chính và cách sử dụng.',
  'Supabase là một nền tảng Backend-as-a-Service (BaaS) mã nguồn mở, được xây dựng trên nền tảng PostgreSQL. Nó cung cấp đầy đủ các tính năng cần thiết cho một backend hiện đại, giúp bạn xây dựng ứng dụng nhanh chóng mà không cần quản lý server phức tạp.
...
(
  gen_random_uuid(),
  (SELECT id FROM profiles LIMIT 1),
  'PostgreSQL vs NoSQL: Chọn cơ sở dữ liệu nào?',
  'postgresql-vs-nosql-chon-co-so-du-lieu-nao',
  'So sánh chi tiết giữa PostgreSQL (SQL) và NoSQL. Hiểu rõ khi nào dùng cái nào để lựa chọn đúng cho dự án của bạn.',
  'Khi xây dựng ứng dụng, một trong những quyết định quan trọng nhất là lựa chọn cơ sở dữ liệu. PostgreSQL (SQL) và NoSQL đều có ưu và nhược điểm riêng.
...
(
  gen_random_uuid(),
  (SELECT id FROM profiles LIMIT 1),
  'Authentication là gì? Cách bảo mật người dùng trong ứng dụng',
  'authentication-la-gi-cach-bao-mat-nguoi-dung',
  'Hiểu rõ về Authentication (xác thực) và Authorization (phân quyền). Cách triển khai một hệ thống login an toàn.',
  'Authentication (Xác thực) là quá trình xác minh danh tính của người dùng. Nó trả lời câu hỏi: "Bạn là ai?"
...
-- 4. Realtime Database là gì?
(
  gen_random_uuid(),
  (SELECT id FROM profiles LIMIT 1),
  'Realtime Database là gì? Cập nhật dữ liệu theo thời gian thực',
  'realtime-database-la-gi-cap-nhat-du-lieu-thuc-chinh',
  'Khám phá công nghệ Realtime Database. Cách triển khai chat, notification, collaborative editing với dữ liệu thay đổi liên tục.',
  'Realtime Database là công nghệ cho phép dữ liệu được cập nhật và đồng bộ giữa server và client theo thời gian thực, không cần refresh trang.
...
-- 5. Backend-as-a-Service (BaaS) là gì?
(
  gen_random_uuid(),
  (SELECT id FROM profiles LIMIT 1),
  'Backend-as-a-Service (BaaS) là gì? Tại sao startup dùng?',
  'baas-backend-as-a-service-la-gi-tai-sao-startup-dung',
  'Hiểu về Backend-as-a-Service. Ưu nhược điểm của BaaS so với tự build backend. Khi nào nên dùng BaaS?',
  'Backend-as-a-Service (BaaS) là mô hình cung cấp cloud service cho phép developer xây dựng ứng dụng mà không cần quản lý backend infrastructure.
...
SELECT 'Thêm 5 bài viết chi tiết thành công!' as result;
